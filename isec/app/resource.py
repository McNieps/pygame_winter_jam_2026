    ...
    def _load_data(cls,
        ...
        for elem in os.scandir(assets_path):
            ...
            if elem.is_file():
                ...
                if elem.name.endswith(".json") or elem.name.endswith(".tmj"):
                    ...
                    value = cls._load_yaml(assets_path+elem.name)
                    if isinstance(value, dict):
                        current_dict[key_name] |= value
                    elif isinstance(value, list) and len(current_dict[key_name]) == 0:
                        current_dict[key_name] = value
                    else:
                        err_msg = f"Mapping already exist for element {elem.name}, cannot concatenate with list"
    ...
    def _load_image(cls,
        ...
        for elem in os.scandir(assets_path):
            ...
            if elem.is_file():
                ...
                elif elem.name.endswith(".pdn") or elem.name.endswith(".gitkeep") or elem.name.endswith(".aseprite"):
                    continue
